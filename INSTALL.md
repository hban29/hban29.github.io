...
### Deployment Profiles (UTK and GitHub Pages)

This repository includes override configuration profiles so you can deploy to different hosts without manually editing `_config.yml` each time.

- UTK hosting profile: `_config.utk.yml`
- GitHub Pages profile: `_config.github-pages.yml`

For UTK hosting (`https://web.eecs.utk.edu/~mdjouadi/`):

```bash
bundle exec jekyll serve --config _config.yml,_config.utk.yml
bundle exec jekyll build --config _config.yml,_config.utk.yml
````

For GitHub Pages (`https://hban29.github.io`):
...
The second config file overrides `url`, `baseurl`, and host-specific footer text.
